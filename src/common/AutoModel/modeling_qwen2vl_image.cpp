...
bool Qwen2VL::ffmpeg_initialized = false;

void Qwen2VL::initialize_ffmpeg() {
...
void Qwen2VL::resolve_source_format_and_range(AVPixelFormat input_format,
...
void Qwen2VL::preprocess_image(qwen2vl_image_t& image, std::vector<bf16> &pixel_values) {
    ...
    if (pad_h > 0 || pad_w > 0) {
        int padded_width = resized_width + pad_w;
        int padded_height = resized_height + pad_h;
        
        std::vector<uint8_t> padded_image(static_cast<size_t>(padded_width) * padded_height * channels, 0); // Initialize with 0
        
        // Copy planar data while verifying bounds
        ...
        for (int c = 0; c < channels; ++c) {
             const uint8_t* src_plane = resize_image.data() + c * src_plane_size;
             uint8_t* dst_plane = padded_image.data() + c * dst_plane_size;
             
             for (int y = 0; y < resized_height; ++y) {
                 std::memcpy(dst_plane + y * padded_width, src_plane + y * resized_width, resized_width);
             }
        }
        
        resize_image = std::move(padded_image);
