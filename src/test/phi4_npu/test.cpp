...
inline std::pair<std::string, std::unique_ptr<AutoModel>> get_phi4_model(const std::string& model_tag) {
    static std::unordered_set<std::string> phi4Tags = {
    ...
    if (phi4Tags.count(model_tag))
        auto_chat_engine = std::make_unique<Phi4>(&npu_device_global);
    else {
        new_model_tag = "phi4-mini-it:4b"; // Default to Phi4 4B
        auto_chat_engine = std::make_unique<Phi4>(&npu_device_global);
    }
...
//     auto [actual_tag, chat] = get_phi4_model(tag);
//     if (actual_tag != tag) {
...
//     phi4_npu phi4_npu_engine(config, &npu, 8192);
//     std::unique_ptr<Q4NX> q4nx = std::make_unique<Q4NX>(model_path);
//     phi4_npu_engine.load_weights(*q4nx);
...
//     buffer<bf16> y = phi4_npu_engine.prefill(tokens, nullptr);
...
int main(int argc, char* argv[]) {
    ...
    chat->load_model(model_path, model_info, 4096, preemption);
    chat->set_topk(5);
    chat->set_temperature(0.6f);
    chat_meta_info_t meta_info;
    lm_uniform_input_t uniformed_input;


    if (short_prompt) {
        uniformed_input.prompt = "Hello, I am a freshman student in EE major.";
