...
inline std::pair<std::string, std::unique_ptr<AutoModel>> get_qwen2vl_model(const std::string& model_tag) {
    static std::unordered_set<std::string> qwen2vl_Tags = {
        "qwen2.5vl-it", "qwen2.5vl-it:3b"
    };

    std::unique_ptr<AutoModel> auto_chat_engine = nullptr;
    std::string new_model_tag = model_tag;
    
    if (qwen2vl_Tags.count(model_tag))
        auto_chat_engine = std::make_unique<Qwen2VL>(&npu_device_global);
    else {
        new_model_tag = "qwen2.5vl-it"; // Default to base Qwen2 model
        auto_chat_engine = std::make_unique<Qwen2VL>(&npu_device_global);
    }
...
int main(int argc, char* argv[]) {
    ...
    auto [actual_tag, chat] = get_qwen2vl_model(tag);
    if (actual_tag != tag) {
